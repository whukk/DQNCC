...
class Sender(object):
    ...
    def __init__(self, port=0, train=False, debug=False):
        ...
        self.q_delay = Queue.Queue(1000)
        self.q_delivery_rate = Queue.Queue(1000)
        self.q_send_rate = Queue.Queue(1000)
        if self.train:
            self.step_cnt = 0
    ...
    def fft_state(self):
        # generate fft state from queue
        q_send_rate_copy = copy.copy(self.q_send_rate)
        send_rate_list = []
        while q_send_rate_copy.not_empty():
            send_rate_list.append(q_send_rate_copy.get())

        q_delay_copy = copy.copy(self.q_delay)
        delay_list = []
        while q_delay_copy.not_empty():
            delay_list.append(q_delay_copy.get())

        q_delivery_rate_copy = copy.copy(self.q_delivery_rate)
        delivery_rate_list = []
        while q_delivery_rate_copy.not_empty():
            delivery_rate_list.append(q_delivery_rate_copy.get())

        # todo : fft for every queue
        queue_list = []
        queue_list.append(send_rate_list)
        queue_list.append(delay_list)
        queue_list.append(delivery_rate_list)

        result_list = []
        for item in queue_list:
            t = np.fft.fft(item) / 1000
            result_list.append(t[range(500)])

        extracted_list = []

        for i in range(len(result_list)):
            temp_index = result_list[i].index(max(result_list[i]))
            temp_value = max(result_list[i])
            extracted_list.append((temp_index, temp_value))
        return extracted_list

    def take_action(self, action_idx):
        old_cwnd = self.cwnd
        op, val = self.action_mapping[action_idx]  # the values are lists ['op', val], such as ['+', '2.0'].
