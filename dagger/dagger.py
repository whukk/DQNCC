    ...
    def wait_on_workers(self):
        ...
        while workers_ep_done < len(self.worker_tasks):
            ...
            for idx in self.worker_tasks:
                ...
                msg = self.sess.run(worker_queue.dequeue())  # used for communicate
...
class DaggerWorker(object):
    def __init__(self, cluster, server, task_idx, env):
        ...
        self.memory = np.zeros((self.memory_size, self.state_dim * 2 + 5 + 1))  # 5=actions;1=reward
    ...
    def setup_tf_ops(self):  # called in __init__()
        ...
        """
        self.prev_state_ph = tf.placeholder(
        ...
        self.cur_state_ph = tf.placeholder(tf.float32,shape=(None,self.aug_state_dim))
    ...
    def sample_action(self, state):
        ...
        transition = np.hstack((aug_state, [self.prev_action, r], self.prev_state))
        ...
        while True:
            ...
            if debug:
                ...
                sys.stderr.write(
                    ...
                    (self.state_buf, self.action_buf, self.state_buf.__len__(), self.action_buf.__len__()))
            ...
            self.sess.run(self.enqueue_train_op, feed_dict={
                self.prev_state_ph: self.memory[:,:self.state_dim],
                self.reward_ph:self.memory[:,self.state_dim],
                self.action_ph:self.memory[:,self.state_dim+1],
                self.cur_state_ph:self.memory[:,-self.state_dim:]
                })
